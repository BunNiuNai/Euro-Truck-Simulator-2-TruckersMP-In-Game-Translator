...
class App:
    ...
    def _open_settings(self):
        ...
        try:
            self._open_settings_webview(old_hotkey)
        except Exception:
            self._open_settings_tkinter(old_hotkey)

    def _open_settings_webview(self, old_hotkey):
        import webview
        import os, sys

        app_cfg = self.cfg
        app_overlay = self.overlay

        class SettingsApi:
            def load_config(self):
                return {
                    "translation_backend": app_cfg.translation_backend,
                    "target_language": app_cfg.target_language,
                    "baidu_appid": app_cfg.baidu_appid,
                    "baidu_secret": app_cfg.baidu_secret,
                    "copy_hotkey": app_cfg.copy_hotkey,
                    "enter_hotkey": app_cfg.enter_hotkey,
                    "send_hotkey": app_cfg.send_hotkey,
                    "window_opacity": app_cfg.window_opacity,
                    "font_size": app_cfg.font_size,
                    "max_messages": app_cfg.max_messages,
                    "player_name": app_cfg.player_name,
                    "window_mode": app_cfg.window_mode,
                    "click_through": app_cfg.click_through,
                }
            def get_providers(self):
                return app_cfg.llm_providers
            def add_provider(self):
                app_cfg.llm_providers.append({"label": f"Provider {len(app_cfg.llm_providers)+1}", "endpoint": "", "api_key": "", "model": "", "enabled": True})
            def remove_provider(self, index):
                if 0 <= index < len(app_cfg.llm_providers):
                    del app_cfg.llm_providers[index]
            def move_provider(self, index, direction):
                ni = index + direction
                if 0 <= ni < len(app_cfg.llm_providers):
                    app_cfg.llm_providers[index], app_cfg.llm_providers[ni] = app_cfg.llm_providers[ni], app_cfg.llm_providers[index]
            def save_config(self, cfg, providers):
                from config import save_config as sc
                app_cfg.translation_backend = cfg["translation_backend"]
                app_cfg.target_language = cfg["target_language"]
                app_cfg.baidu_appid = cfg["baidu_appid"]
                app_cfg.baidu_secret = cfg["baidu_secret"]
                app_cfg.copy_hotkey = cfg["copy_hotkey"]
                app_cfg.enter_hotkey = cfg["enter_hotkey"]
                app_cfg.send_hotkey = cfg["send_hotkey"]
                app_cfg.window_opacity = float(cfg["window_opacity"])
                app_cfg.font_size = int(cfg["font_size"])
                app_cfg.max_messages = int(cfg["max_messages"])
                app_cfg.player_name = cfg["player_name"]
                app_cfg.window_mode = cfg["window_mode"]
                app_cfg.click_through = bool(cfg["click_through"])
                app_cfg.llm_providers = providers
                if providers:
                    app_cfg.api_endpoint = providers[0].get("endpoint", "")
                    app_cfg.api_key = providers[0].get("api_key", "")
                    app_cfg.api_model = providers[0].get("model", "")
                sc(app_cfg)
                app_overlay.set_opacity(app_cfg.window_opacity)
                app_overlay.set_font_size(app_cfg.font_size)
                app_overlay.cfg = app_cfg
                app_overlay._apply_mode()
            def test_all(self, providers):
                ...
                if app_cfg.baidu_appid and app_cfg.baidu_secret:
                    ok, msg = test_baidu_connection(app_cfg.baidu_appid, app_cfg.baidu_secret)
        ...
        w = webview.create_window(
            ...
            js_api=SettingsApi(),
            width=app_cfg.settings_win_w if app_cfg.settings_win_w > 0 else 780,
            height=app_cfg.settings_win_h if app_cfg.settings_win_h > 0 else 700,
            min_size=(600, 500),
        )
        webview.start()
        # After webview returns (window closed by save/cancel), sync back
        if app_cfg.send_hotkey != old_hotkey:
            app_overlay.update_send_hotkey(app_cfg.send_hotkey)

    def _open_settings_tkinter(self, old_hotkey):
...
class HotkeyCapture(tk.Frame):
    ...
    def _start_capture(self, event=None):
        ...
        self._label.config(text="按下组合键...", bg="#2563eb", fg="#ffffff",
...
class SettingsDialog:
    ...
    def _switch_tab(self, key: str) -> None:
        ...
        for f in self._tab_frames.values():
            f.grid_remove()
        # Activate selected
        self._tabs[key].configure(bg="#2563eb", fg="#ffffff")
